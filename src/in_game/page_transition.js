...
const planetImgSrc = [
  "./img/new_img/gs_kul_house.png",
  "./img/new_img/cu_library.png",
  "./img/new_img/cu_hak.png",
];
const bossImgSrc = [
...
const attackImgSrc = [
  "./img/in_game/rock.png",
  "./img/in_game/flash.png",
  "./img/in_game/meteor.png",
];
...
const setUserHP = (hp) => {
  ...
  const empty_heart =
    '<img src="./img/in_game/empty-heart.png" width="60px" />';
...
const paddleDrawing = () => {
  ...
  context.rect(
    paddleStatus.posX,
    canvas.height - paddleStatus.height,
    paddleStatus.width,
    paddleStatus.height
  );
...
const setBricks = (stageLevel) => {
  ...
  if (stageLevel != 3) {
    while (
      blockCount <
      brickContainer.bricksOnColumn * brickContainer.bricksOnRow
    ) {
    ...
    while (
      ...
      if (
        ...
        blockCount % brickContainer.bricksOnRow >=
          (brickContainer.bricksOnRow - 3) / 2 && // 가운데 3개의
        blockCount % brickContainer.bricksOnRow <
          (brickContainer.bricksOnRow - 3) / 2 + 3 // column에 속하는가?
      ) {
...
const setBrickSize = () => {
  normalBrick.width =
    ($("#game-page__game-scene").width() -
      (2 * brickContainer.offsetSide +
        brickContainer.spaceBetweenBricks * (brickContainer.bricksOnRow - 1))) /
    brickContainer.bricksOnRow;

  bossBrick.width =
    normalBrick.width * 3 + brickContainer.spaceBetweenBricks * 2;
  bossBrick.height =
    normalBrick.height * 3 + brickContainer.spaceBetweenBricks * 2;
};
...
const wallCollisionDetect = () => {
  ...
  // 부딛히지 않은 경우에는 0반환
  if (
    ballStatus.posY + ballStatus.dy > canvas.height - ballStatus.radius ||
    ballStatus.posY + ballStatus.dy < 0
  ) {
    return 1;
  } else if (
    ballStatus.posX + ballStatus.dx > canvas.width - ballStatus.radius ||
    ballStatus.posX + ballStatus.dx < 0
  ) {
    return -1;
  } else {
    return 0;
...
const paddleCollisionDetect = () => {
  ...
  if (
    paddleStatus.posX <= ballStatus.posX &&
    paddleStatus.posX + paddleStatus.width >= ballStatus.posX
  ) {
    if (
      canvas.height - paddleStatus.height <
      ballStatus.posY + ballStatus.radius / 2
    ) {
      console.log("hit paddle!!!");
      return true;
    }
...
const attackCollisionDetect = (attack) => {
  if (
    attack.posX > paddleStatus.posX + paddleStatus.width ||
    attack.posX + bossAttack.width < paddleStatus.posX
  ) {
    return false;
  }
  return true;
...
const setGameInitialStatus = () => {
  ...
  // 4. 탐색 행성 사진
  $("#game-page__current-planet img:first").attr(
    "src",
    planetImgSrc[gameStatus.gameLevel - 1]
  );
